...
static FC_Font* getFontFromMap(int size)
{
    ...
    if (got == s_fonts.end() || got->second == NULL) {
        ...
        FC_LoadFont_RW(f, s_renderer, SDL_RWFromMem(fontData, fontSize), 1, size, COLOR_BLACK, TTF_STYLE_NORMAL);
...
{
    SDL_Surface* loaded_surface = NULL;
    loaded_surface              = tga ? IMG_LoadTGA_RW(SDL_RWFromMem(buff, size)) : IMG_Load_RW(SDL_RWFromMem(buff, size), 1);
